...
- **Single-pass in-memory indexing (SPIMI)**:
    - Uses terms instead of `termID`s: No need to maintain term-`termID` mapping across blocks.
    - Writes each block's *dictionary* (best implemented as hash) to disk, then starts a new dictionary for the next block
        - As long as they fit into the disk, SPIMI can index collections of any size.
...
- *Map Phase*: Mapping splits of the input data to key-value pairs.
    ...
    - Have the parsers write key-value pairs for each term partition into *a separate segment file*.
        Example: The term partitions according to first letter: `a-f`, `g-p`, `q-z`. (So here, $j=3$. In general, key ranges need not correspond to contiguous terms or `termID`s.)
...
- Parsers and inverters are not separate sets of physical machines - the master identifies idle machines and assigns tasks to them.
...
- If new documents need to be available quickly, maintain *two* indexes: a large *main* index and a small *auxiliary* index that stores new documents
    ...
    - If we store *each postings list* as a separate file, then the merge simply consists of extending each postings list of the main index by the corresponding postings list of the auxiliary index.
        - Updating each document separately requires up to $M_{\text{ave}}$ disk seeks, where $M_{\text{ave}}$ is the average size of the vocabulary of documents in the collection.
...
- When we use one large file, we process each posting $\lfloor T/n \rfloor$ times as
    - We touch it during each of $\lfloor T/n \rfloor$ merges where $n$ is the size of the auxiliary index and `T`, the total number of postings.
...
- We can do better than $\Theta(T^2/n)$ by introducing $\log_2 (T/n)$ indexes.
    - That is, instead of doing $\lfloor (T/n) \rfloor$ merges straight, we could do *logarithmic merging*.
    - Indexes $I_0, I_1, \cdots, I_n$ have sizes of $2^0 \cdot n, 2^1 \cdot n, 2^2 \cdot n$, and so on.
    - Postings percolate up this sequence of indexes and process only once on each level.
        ...
        - When the limit $n$ is reached, the $2^0 \cdot n$ postings in $Z_0$ are transferred to $I_0$ (This one is stored on *disk*). **And we empty $Z_0$**. 
        - The next time $Z_0$ is full, it is merged with $I_0$ to create an index $Z_1$ of size $2^1 \cdot n$.
